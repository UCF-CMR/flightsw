//This code is meant to test single COLLIDE on a level tabletop in a 1g environment.
//Written by Richard Wakefield
//July 2020

// DIFFERENCES BETWEEN THIS AND THE FLIGHT CODE:
// 1) There is a "-1" added into line 320 to manually subtract gravity.  This makes the experiment think it is in freefall while sitting on a level table.
// 2) The time between projectile launching has been decreased from 103 seconds to 10 seconds, so that we don't have to wait excessively long. (See line 265)
...
void Experiment()
{
    ...
    delay(10000);                                         // and is then deactivated. The 103.0 second delay
...
void GetPacket()
{
  ...
  averageAccel[2] = (averageAccel[2] / 8192)-1;
