...
void state_transition(int new_state, unsigned long delay_time)
{
  ...
  datalog("Transitioning to state ");
  ...
  switch(new_state)
  {
    ...
    case STATE_DELAY:
      datalog("DELAY [");
      datalog(String(delay_time));
      datalogln("]");
      break;
    case STATE_INITIALIZE:         datalogln("INITIALIZE");         break;
    case STATE_TRIGGER_WAIT:       datalogln("TRIGGER_WAIT");       break;
    case STATE_TRIGGER_VERIFY:     datalogln("TRIGGER_VERIFY");     break;
    case STATE_12V_REG_ENABLE:     datalogln("12V_REG_ENABLE");     break;
    case STATE_12V_REG_DISABLE:    datalogln("12V_REG_DISABLE");    break;
    case STATE_HV_REG_ENABLE:      datalogln("HV_REG_ENABLE");      break;
    case STATE_HV_REG_DISABLE:     datalogln("HV_REG_DISABLE");     break;
    case STATE_ELECTROMETER_START: datalogln("ELECTROMETER_START"); break;
    case STATE_ELECTROMETER_STOP:  datalogln("ELECTROMETER_STOP");  break;
    case STATE_STEPPER_RUN_OPENED: datalogln("STEPPER_RUN_OPENED"); break;
    case STATE_STEPPER_RUN_CLOSED: datalogln("STEPPER_RUN_CLOSED"); break;
    case STATE_IDLE:               datalogln("IDLE");               break;
    case STATE_TERMINATE:          datalogln("TERMINATE");          break;
    default:                       datalogln("UNKNOWN");            break;
...
void door_state_transition(int new_state)
{
  ...
  datalog("Transitioning to door state ");

  switch(new_state)
  {

    case DOOR_OPENING: datalogln("OPENING"); break;
    case DOOR_OPENED:  datalogln("OPENED");  break;
    case DOOR_CLOSING: datalogln("CLOSING"); break;
    case DOOR_CLOSED:  datalogln("CLOSED");  break;
    default:           datalogln("UNKNOWN"); break;
...
void sd_card_start()
{
  ...
  if(sd_card_enable)
  {
    ...
    do
    {
      ...
      sd_card_fileindex++;
      sprintf(sd_card_filename, "%05d.txt", sd_card_fileindex);

    }
    while(SD.exists(sd_card_filename));
...
void loop()
{
  ...
  switch(state)
  {
    ...
    case STATE_STEPPER_RUN_OPENED:
      datalogln("Running stepper in opened direction");
    ...
    case STATE_STEPPER_RUN_CLOSED:
      datalogln("Running stepper in closed direction");
      digitalWrite(PIN_DO_STEP_ENABLE, HIGH);
      digitalWrite(PIN_DO_STEP_DIR,    LOW);
      start_stepper();
