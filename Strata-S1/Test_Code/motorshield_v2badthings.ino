...
// NOT A LONG TERM SOLUTION FIX LATER IN ABOVE PIN DEFINITIONS
const char PIN_EN_ARR[] = {ENABLE_D, ENABLE_C, ENABLE_B, ENABLE_A};
const char PIN_A1_ARR[] = {MD_A1,    MC_A1,    MB_A1,    MA_A1   };
const char PIN_A2_ARR[] = {MD_A2,    MC_A2,    MB_A2,    MA_A2   };
const char PIN_PS_ARR[] = {POS_D,    POS_C,    POS_B,    POS_A   };

// Arrays to hold current positions for actuators
int act_pos[]   = {  0,   0,   0,   0};
int act_max[]   = {  0,   0,   0,   0};
int act_maxe[]  = { 75,  75,  75,  75};
int act_mine[]  = {400, 400, 400, 750};
  ...
  if( digitalRead(TRIGGER) == HIGH )
  {
    ...
    if( digitalRead(TRIGGER) == HIGH )
    {
      ...
      for( char i = 0; i < 4; i++ )
      {
        ...
        digitalWrite(PIN_EN_ARR[i], HIGH);
        unsigned long start = millis();
        while( act_pos[i] < act_mine[i] && millis() - start < timeout )
        {
          act_pos[i] = analogRead(PIN_PS_ARR[i]);
          Serial.print(i, DEC);
