...
//*************////*************////*************//
// Variables to change for rotation
// - code below is set to run by # steps, so use those variables instead of rotation degrees
// nsteps = deg_rotation / 1.8

// change these variables first
//int initdeg   = 45 // variable for initial rotation
int initsteps = 25 // variable for # steps in initial rotation
//int incdeg    =  5 // variable for incremental rotation
int incsteps  = 3

// maybe play around with these variables later
int initrpm  = 10   // varaiable for initial speed
int incrpm   = 10   // variable for incremental speed
//*************////*************////*************//
 ...
 */
void SUB_ROTATE(int n_steps, int rpm){
 ...
 */
void SUB_ITERATE(int n_stepsit, int rpmit){

  Serial.println("\t #SUB_ITERATE #");  
  int   i = 0;
  float dtime = 3000 / (20 * rpmit);  
  digitalWrite(DIR, LOW);     
  delay(1);
  
  for(i=0; i < n_stepsit ; i++){          // Iterate 5.4 deg or 3 steps
       digitalWrite(PUL, HIGH);
